...
class Cache:
    ...
    def __contains__(self, key):
        return self.make_key(key) in redis_client

    def get(self, key, default=None):
        value = redis_client.get(self.make_key(key))
        if value is None:
            return default
    ...
    def set(self, key, value, timeout=None):
        ...
        return redis_client.set(self.make_key(key), value, ex=timeout)

    def add(self, key, value, timeout=None):
        timeout = timeout or self.default_timeout
        value = self._dump_object(value)
        return redis_client.set(self.make_key(key), value, ex=timeout, nx=True)

    def delete(self, key):
        return redis_client.delete(self.make_key(key)) == 1
